...
import { applyNodeChanges, getConnectedEdges } from "@xyflow/react";

import { doc } from "@/data/ydoc";
import { edgesMap } from "./useEdgeStateSynced";
...
export const nodesMap = doc.getMap("nodes");

function useNodesStateSynced() {
  const [nodes, setNodes] = useState([]);

  const setNodesSynced = useCallback((nodesOrUpdater) => {
    const seen = new Set();
    const next =
